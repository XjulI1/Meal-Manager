...
Pas de `sourceUrl` (recette papier). `max_tokens` borné comme l'URL ; effort **`low`** par défaut — l'extraction est forcée via l'outil `extract_recipe` et les recettes ciblées sont des textes imprimés (livres) où `low` est rapide et suffisant ; surchargeable en `medium`/`high` via `NUXT_ANTHROPIC_PHOTO_EFFORT` pour des cas plus difficiles (manuscrit).
...
### D8 — HEIC : conversion côté client (pas de support serveur)
L'API vision de Claude n'accepte que **JPEG, PNG, GIF, WebP** — **pas le HEIC**, qui est pourtant le format par défaut de l'appareil photo iPhone. Pour le supporter, le front convertit les fichiers HEIC/HEIF en **JPEG dans le navigateur** via `heic-to` (libheif récent compilé en WASM) avant l'encodage base64 : `useApiRecipeChat.toApiImage()` détecte le HEIC (octets magiques via `isHeic()`, avec repli type MIME `image/hei[cf]` / extension `.heic`/`.heif`) et le convertit ; les autres formats passent tels quels.

Rationale : le serveur et le **contrat Anthropic restent inchangés** (le DTO/adapter ne voient jamais que les trois formats web), aucune modification de l'image Docker (pas de libheif natif), et la conversion s'insère naturellement dans le `importPhotos` qui lit déjà les fichiers côté client. On utilise `heic-to` plutôt que `heic2any` : ce dernier embarque une libheif ancienne qui échoue (`heif_error_Invalid_input`) sur les HEIC « grid »/tuilés des iPhone récents ; `heic-to` (même auteur, successeur maintenu) embarque une libheif à jour, fournit `isHeic()` basé sur les octets magiques et ses propres types TS. **Alternative écartée** : conversion serveur (`heic-convert` ou `sharp`+libheif). Rejetée car elle impose une dépendance native dans le conteneur (libheif), élargit le DTO à `image/heic`, et alourdit le payload (HEIC brut en base64). Coût accepté côté client : poids du WASM et décodage CPU sur l'appareil (lent sur de très gros HEIC). `heic-to` est **dynamiquement importé** (`await import('heic-to')`) pour rester hors du bundle SSR et n'être chargé que lors d'un import photo ; un échec de conversion lève une erreur lisible affichée en toast.

### D9 — Redimensionnement client avant l'envoi
Chaque image (après conversion HEIC le cas échéant) est **redimensionnée côté navigateur** via un `canvas` à un plus grand côté ≤ **1600 px** puis ré-encodée en JPEG (qualité 0,85) avant l'encodage base64. Motivation : une photo iPhone pleine résolution (~12 Mpx) ré-encodée en JPEG dépasse la borne `MAX_RECIPE_PHOTO_BASE64_CHARS` et fait échouer le `safeParse` Zod (HTTP 400). Le redimensionnement garde le payload petit (~quelques centaines de Ko/image), reste largement sous la borne, et **réduit le coût en tokens vision** — l'API sous-échantillonne les grandes images de toute façon, 2000 px suffisent pour l'OCR d'une recette. Un repli envoie la source telle quelle si le chemin `canvas` est indisponible.
