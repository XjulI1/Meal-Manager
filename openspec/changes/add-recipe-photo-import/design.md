...
L'API vision de Claude n'accepte que **JPEG, PNG, GIF, WebP** — **pas le HEIC**, qui est pourtant le format par défaut de l'appareil photo iPhone. Pour le supporter, le front convertit les fichiers HEIC/HEIF en **JPEG dans le navigateur** via `heic2any` (libheif compilé en WASM) avant l'encodage base64 : `useApiRecipeChat.toApiImage()` détecte le HEIC (type MIME `image/hei[cf]` ou extension `.heic`/`.heif`) et le convertit ; les autres formats passent tels quels.

Rationale : le serveur et le **contrat Anthropic restent inchangés** (le DTO/adapter ne voient jamais que les trois formats web), aucune modification de l'image Docker (pas de libheif natif), et la conversion s'insère naturellement dans le `importPhotos` qui lit déjà les fichiers côté client. **Alternative écartée** : conversion serveur (`heic-convert` ou `sharp`+libheif). Rejetée car elle impose une dépendance native dans le conteneur (libheif), élargit le DTO à `image/heic`, et alourdit le payload (HEIC brut en base64). Coût accepté côté client : poids du WASM `heic2any` et décodage CPU sur l'appareil (lent sur de très gros HEIC). `heic2any` est **dynamiquement importé** (`await import('heic2any')`) pour rester hors du bundle SSR et n'être chargé que lors d'un import photo.
