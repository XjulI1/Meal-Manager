...
- [x] 7.6 Redimensionnement client (canvas, plus grand côté ≤ 1600 px, ré-encodage JPEG) avant l'envoi, pour rester sous la borne de taille et limiter le coût en tokens vision
- [x] 7.7 Performance : effort photo par défaut `low` (recettes imprimées) ; retour visuel par étapes (« Préparation… » / « Interprétation… ») pendant l'import
