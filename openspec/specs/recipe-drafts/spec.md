# recipe-drafts Specification

## Purpose
Persist work-in-progress recipe drafts per household, fed automatically by the web recipe form (autosave, no save button) and by AI/MCP producers, then promoted into published recipes via the existing resolve + create flow.
## Requirements
